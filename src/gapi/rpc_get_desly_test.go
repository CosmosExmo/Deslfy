...
func TestGetDeslyGRPC(t *testing.T) {
	user, _ := randomUser(t)
	desly := randomDesly(user.Username)

	invalidDesly := util.RandomString(5)

	testCases := []struct {
		name          string
		req           *pb.GetDeslyRequest
		buildStubs    func(store *mockdb.MockStore)
		buildContext  func(t *testing.T, tokenMaker token.Maker) context.Context
		checkResponse func(t *testing.T, res *pb.GetDeslyResponse, err error)
	}{
		{
			name: "OK",
			req: &pb.GetDeslyRequest{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
					GetDesly(gomock.Any(), gomock.Eq(arg)).
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			res, err := server.GetDesly(ctx, tc.req)
